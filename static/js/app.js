...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para alternar visibilidad de Sidebar y Overlay
    ...
    // Asignar eventos al botón menú hamburguesa
    if (mobileMenuBtn) {
        mobileMenuBtn.addEventListener('click', toggleSidebar);
        mobileMenuBtn.addEventListener('touchstart', toggleSidebar, { passive: false });
    }

    // Asignar evento al overlay para cerrar menú al tocar afuera
    if (sidebarOverlay) {
        sidebarOverlay.addEventListener('click', toggleSidebar);
        sidebarOverlay.addEventListener('touchstart', toggleSidebar, { passive: false });
    }

    // Cierre de alertas personalizadas
    const closeAlertButtons = document.querySelectorAll('.btn-close-custom');
    closeAlertButtons.forEach(button => {
        button.addEventListener('click', () => {
            const alertContainer = button.closest('.custom-alert');
            if (alertContainer) {
